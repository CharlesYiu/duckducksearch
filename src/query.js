        ...
        const parser = new htmlparser2.Parser({
            onopentag(name, attributes) {
                ...
                if (name === "div" && classList.includes("result__body")) {
                    ...
                    if (
                        classList.includes("result__a") &&
                        result.title === null
                    )
                        scrape = "title";
                    else if (
                        classList.includes("result__url") &&
                        result.url === null
                    )
                        scrape = "url";
                    else if (
                        classList.includes("result__snippet") &&
                        result.snippet === null
                    )
                        scrape = "snippet";
                }
        ...
        fetch(
            `https://html.duckduckgo.com/html/?q=${encodeURIComponent(text)}`,
            {
                headers: {
                    ...
                    "User-Agent":
                        "Mozilla/5.0 (X11; Linux aarch64; rv:107.0) Gecko/20100101 Firefox/107.0",
                },
            }
        )
            .then((response) => {
...
// query arg if running from cmd
if (process.argv.length > 2)
    query(process.argv.slice(2).join(" ")).then((results) =>
        console.log(results)
    );
